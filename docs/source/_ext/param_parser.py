"""Optional autodoc processor for dash-listed parameter sections.

Not enabled by default (mirrors rheedium, where this extension exists but is
not in ``extensions``). Add ``"param_parser"`` to ``extensions`` in conf.py to
activate it for docstrings that list parameters as ``- name (type): desc``.
"""
